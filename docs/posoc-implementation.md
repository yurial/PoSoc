...
- **Канал уведомлений (опционально):** $s_{uv} = \mathrm{HKDF}(\mathrm{X25519}(x_u, pk_v))$ — координация продлений и подписей `LINK`; не источник истины.

Объёмы записей: `LINK` ≈ 200 Б; одиночные объекты и revoke ≈ 150 Б; `MEMBER_OF_BATCH(30)` ≈ 1 КБ. Подписная нагрузка на человека скромна — порядка 80–100 тихих записей в год: ≈ 60 продлений `LINK` + 2 само-членства `MEMBER_OF` + 2 пакета подтверждений + ~$2D \approx 16\text{–}20$ declarations (специфицируемая часть этой нагрузки — cadence TTL, то есть значения из [09_parameters.md](../specs/concept/09_parameters.md)). Пропускная способность, политика relay и объём хранилищ — вне спецификации (implementation-defined).
...
**Транспорт = социальный граф.** Запись ретранслируется только узлам с активной `LINK`; свежий ключ без связей не получает трафика, а установление `LINK` включает слив состояния. Компонент связности — изолированная вселенная данных: записи человеческого компонента физически не достигают бот-кластеров (двойная изоляция — структурная и транспортная; защитный смысл этого свойства разобран в концепции).
