...
Объёмы записей: `FRIEND` ≈ 200 Б; одиночные объекты и revoke ≈ 150 Б; `VOUCH_BATCH(30)` ≈ 1 КБ. Подписная нагрузка на человека скромна — порядка 80–100 тихих записей в год: ≈ 60 продлений `FRIEND` + 2 `SELF_CONF` + 2 vouch-пакета + ~$2D \approx 16\text{–}20$ declarations (специфицируемая часть этой нагрузки — cadence TTL, то есть значения из [09_parameters.md](../specs/concept/09_parameters.md)). Пропускная способность, политика relay и объём хранилищ — вне спецификации (implementation-defined).
