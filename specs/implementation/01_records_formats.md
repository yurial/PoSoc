...
`MEMBER_OF` — «X member of G»: `member` — член $X$; подписант = $X$ — само-членство (его revoke — выход), подписант ≠ $X$ — подтверждение другого (его revoke — отзыв подтверждения). Подписант в payload не дублируется — он определяется подписью в sigs (по образцу `LINK {u, v}`: стороны — в payload, подписанты — в sigs).
